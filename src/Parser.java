...
public class Parser{
	...
	for(int r = 0; r < 100000; r++){
		...
		// TODO: there must be a neater way to iterate over a byteStream,
		// than to simply keep reading until an exception is thrown.
    ...
    void postParsing(ArrayList<Poly> polys, ArrayList<String> names,
		     ArrayList<String> visited, ArrayList<String> info){
	...
	for(int i = 0; i < polys.size(); i++){
		...
		if(str.length == 4){
		    ...
		    polys.get(i).cities.add(new City(str[0], x, y, str[3]));
		}else if(str.length == 2){
		    polys.get(i).cities.add(new City(str[0], 0, 0, str[1]));
		}
	    }
	}
